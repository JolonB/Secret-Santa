...
if __name__ == "__main__":
    ...
    if recipient == gifter:
        print(
            ...
            " so the best option is to reallocate everyone.".format(player_to_remove),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if filename:
        new_allocation_str = store_allocation.encode_data(
            allocations_dict, "new_{}".format(filename)
        )
    else:
        new_allocation_str = store_allocation.encode_data(
            allocations_dict, "new_allocations"
        )
